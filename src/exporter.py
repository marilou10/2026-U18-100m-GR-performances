...
def output_filename(output_dir):
    ...
    return os.path.join(output_dir, f"2026_U18_100m_GR_{timestamp}.xlsx")
...
def create_excel(all_results, ranking, wind_legal_ranking, wind_aided_ranking, fmt_note, fmt_wind, fmt_comp, fmt_loc):
    ...
    sorted_all = sorted(all_results, key=lambda x: float(x["performance"].replace("w","").replace("h","")) if x.get("performance","").replace("w","").replace("h","").replace(".","").isdigit() else 999.0)
...
def export_csv(all_results, filename, fmt_wind, fmt_comp, fmt_loc, fmt_note):
    ...
    with open(csv_filename, 'w', encoding='utf-8', newline='') as f:
        w = csv.writer(f)
        w.writerow(G)
        for r in sorted(all_results, key=lambda x: float(x["performance"].replace("w","").replace("h","")) if x.get("performance","").replace("w","").replace("h","").replace(".","").isdigit() else 999.0):

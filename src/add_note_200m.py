...
#
# Notes manager for 2026 U18 200m Greek performances.

"""
Manage notes on performance entries.
Notes are stored in cache_notes_200m.json and populate the ΣΗΜΕΙΩΣΕΙΣ column.

Usage:
  python src/add_note_200m.py                    Interactive: search, pick entries, add/remove notes
  python src/add_note_200m.py list               List all entries with notes
  python src/add_note_200m.py search <term>      Search entries in the cache
  python src/add_note_200m.py add <key> <note>   Add a note by exact key
  python src/add_note_200m.py remove <key>       Remove a note by exact key
  python src/add_note_200m.py filter [opts]      Filter entries (--club, --name, --min-perf, etc.)
"""

import json, os, sys, re
from config_200m import GREEK_TO_LATIN, BASE

CACHE_FILE = os.path.join(BASE, "cache_performances_200m.json")
NOTES_FILE = os.path.join(BASE, "cache_notes_200m.json")
...
def cmd_filter(args):
    ...
    p = _a.ArgumentParser(prog="add_note_200m.py filter", add_help=False)
    p.add_argument("--club")
    p.add_argument("--name")
    ...
    if matches:
        ...
        print(f"\nFirst entry key: {k}")
        print(f'  Use: python src/add_note_200m.py add "{k}" "your note"')
...
HELP_TEXT = """
  ...
  filter [opts]   - show entries matching criteria
    ...
    --min-perf TIME   minimum time (e.g. 24.0)
